...
/**
 * Returns an array of tokens, one per top-level entry in LIST_TOKENS.
 * If a token has multiple chain variants, we pick the NEAR variant first.
 */
export function getUnifiedTokenList(): UnifiedAsset[] {
  return LIST_TOKENS.map((tokenEntry) => {
    // If it has groupedTokens, choose one
    if (
      Array.isArray(tokenEntry.groupedTokens) &&
      tokenEntry.groupedTokens.length > 0
    ) {
      // Attempt to find the NEAR version
      const nearVariant = tokenEntry.groupedTokens.find(
        (t) => t.chainName?.toLowerCase() === "near",
      );
      const chosen = nearVariant || tokenEntry.groupedTokens[0];

      return {
        defuse_asset_id: chosen.defuseAssetId,
      ...
      return {
        defuse_asset_id: tokenEntry.defuseAssetId || "",
